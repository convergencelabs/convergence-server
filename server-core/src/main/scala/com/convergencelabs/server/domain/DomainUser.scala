...
case class DomainUserId(
  ...
  def isAnynomousl(): Boolean = this.userType == DomainUserType.Anonymous
}
...
object DomainUser {
  def apply(
    ...
    email: Option[String]): DomainUser = DomainUser(userId.userType, userId.username, firstName, lastName, displayName, email, false, false, None)
  ...
  def apply(
    ...
    DomainUser(userType, username, firstName, lastName, displayName, email, false, false, None)
}
