  ...
  "A ModelQueryBuilder" when {
    ...
    "given 1 order by" must {
      "return correct ModelQueryParameters" in {
        ...
        ModelQueryBuilder.queryModels(select) shouldBe
          ModelQueryParameters(
            "SELECT FROM Model WHERE collection.id = :0 ORDER BY data.someField ASC",
            Map("0" -> "myCollection"))
      }
    }
    "given multiple order bys" must {
      "return correct ModelQueryParameters" in {
        ...
        ModelQueryBuilder.queryModels(select) shouldBe
          ModelQueryParameters(
            "SELECT FROM Model WHERE collection.id = :0 ORDER BY data.someField ASC, data.anotherField DESC",
            Map("0" -> "myCollection"))
      }
    }
    ...
    "given an In where clause" must {
      "return correct ModelQueryParameters" in {
        ...
        ModelQueryBuilder.queryModels(select) shouldBe
          ModelQueryParameters(
            "SELECT FROM Model WHERE collection.id = :0 and (data.name in :1)",
    ...
    "given a Like where clause" must {
      "return correct ModelQueryParameters" in {
        ...
        ModelQueryBuilder.queryModels(select) shouldBe
          ModelQueryParameters(
            "SELECT FROM Model WHERE collection.id = :0 and (data.name like :1)",
