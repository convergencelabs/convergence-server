  ...
  "A ArrayMoveReplaceTF" when {
    "tranforming an forward move against an replace" must {
      ...
      "transform neither operation if the replace is before the move." in {
        ...
        val c = ArrayReplaceOperation(Path, false, 2, JString("X"))
       ...
       *
       * Server Op       :           ^-->--^                   Move(3, 5)
       * Client Op       :           ^                         Replace(3, X)
       *
       * Server State    : [A, B, C, E, F, D, G, H, I, J]
       * Client Op'      :                 ^                   Replace(5, X)
       *
       * Client State    : [A, B, C, D, E, F, G, H, I, J]
       * Server Op'      :           ^-->--^                   Move(3, 5)
       *
      ...
      "no transform the move and set the replace index to the move's toIndex, if the replace is at the start of the move" in {
        val s = ArrayMoveOperation(Path, false, 3, 5)
        val c = ArrayReplaceOperation(Path, false, 3, JString("X"))

        val (s1, c1) = ArrayMoveReplaceTF.transform(s, c)

        s1 shouldBe s
        c1 shouldBe ArrayReplaceOperation(Path, false, 5, JString("X"))
      }
       ...
       *
       * Server Op       :           ^-->--^                   Move(3, 5)
       * Client Op       :              ^                      Replace(4, X)
       *
       * Server State    : [A, B, C, E, F, D, G, H, I, J]
       * Client Op'      :           ^                         Replace(3, X)
       *
       * Client State    : [A, B, C, D, X, F, G, H, I, J]
       * Server Op'      :           ^-->--^                   Move(3, 5)
       *
      ...
      "decrement the replace index and not transform the move, if the replace in the middle of the move" in {
        val s = ArrayMoveOperation(Path, false, 3, 5)
        val c = ArrayReplaceOperation(Path, false, 4, JString("X"))

        val (s1, c1) = ArrayMoveReplaceTF.transform(s, c)

        s1 shouldBe ArrayMoveOperation(Path, false, 3, 5)
        c1 shouldBe ArrayReplaceOperation(Path, false, 3, JString("X"))
      }
       ...
       * Client Op'      :              ^                      Replace(4, X)
       *
       * Client State    : [A, B, C, D, E, X, G, H, I, J]
       * Server Op'      :           ^-->--^                   Move(3, 5)
       *
      ...
      "not transform the move and decrement the replace index, if the replace is at the end of the move" in {
        val s = ArrayMoveOperation(Path, false, 3, 5)
        val c = ArrayReplaceOperation(Path, false, 5, JString("X"))

        val (s1, c1) = ArrayMoveReplaceTF.transform(s, c)

        s1 shouldBe s
        c1 shouldBe ArrayReplaceOperation(Path, false, 4, JString("X"))
      }
       ...
       *
       * Server Op       :           ^-->--^                     Move(3, 5)
       * Client Op       :                    ^                  Replace(6, X)
       *
       * Server State    : [A, B, C, E, F, D, G, H, I, J]
       * Client Op'      :              ^                        Replace(6, X)
       *
       * Client State    : [A, B, C, D, E, F, X, H, I, J]
       * Server Op'      :           ^-->--^                     Move(3, 5)
       *
      ...
      "transform neither operation, if the replace is after the move" in {
        val s = ArrayMoveOperation(Path, false, 3, 5)
        val c = ArrayReplaceOperation(Path, false, 6, JString("X"))
    ...
    "tranforming a identity move against an replace" must {
      ...
      "transform neither operation if the replace is before the move." in {
        ...
        val c = ArrayReplaceOperation(Path, false, 1, JString("X"))
