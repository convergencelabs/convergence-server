...
class AuthenticationHandlerSpec()
  ...
  "A AuthenticationHandler" when {
    ...
    "authenticating a user by token" must {
      ...
      "return an authentication failure for a non-existent key" in new TestFixture {
        val f = authHandler.authenticate(TokenAuthRequest(JwtGenerator.generate(existingUserName, missingKey)))
        val result = Await.result(f, FiniteDuration(1, TimeUnit.SECONDS))
        result shouldBe AuthenticationFailure
      }

      "return an authentication failure for a disabled key" in new TestFixture {
        val f = authHandler.authenticate(TokenAuthRequest(JwtGenerator.generate(existingUserName, disabledKey.id)))
        val result = Await.result(f, FiniteDuration(1, TimeUnit.SECONDS))
        result shouldBe AuthenticationFailure
      }

      "return an authentication failure for an invalid key" in new TestFixture {
        val f = authHandler.authenticate(TokenAuthRequest(JwtGenerator.generate(existingUserName, invalidKey.id)))
        val result = Await.result(f, FiniteDuration(1, TimeUnit.SECONDS))
        result shouldBe AuthenticationFailure
      }
  ...
  trait TestFixture {
    ...
    val lazyUser = DomainUser("uid-b3", lazyUserName, None, None, None)
    ...
    val brokenLazyUser = DomainUser("uid-b2", brokenLazyUsername, None, None, None)
