...
class DomainStoreSpec
    extends PersistenceStoreSpec[DomainStoreSpec.SpecStores]("/convergence.json.gz")
    with WordSpecLike
    with Matchers {

  def createStore(dbPool: OPartitionedDatabasePool): DomainStoreSpec.SpecStores = {
    DomainStoreSpec.SpecStores(new UserStore(dbPool, Duration.ofSeconds(1)), new DomainStore(dbPool))
  }
