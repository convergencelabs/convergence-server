...
class ModelStoreSpec
    extends PersistenceStoreSpec[ModelStoreSpecStores]("/domain.json.gz")
    with WordSpecLike
    with Matchers {

  val df = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSz")

  def createStore(dbPool: OPartitionedDatabasePool): ModelStoreSpecStores = {
    val modelStore = new ModelStore(dbPool, new ModelOperationStore(dbPool), new ModelSnapshotStore(dbPool))
    val collectionStore = new CollectionStore(dbPool, modelStore)
