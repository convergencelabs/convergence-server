...
import com.convergencelabs.convergence.server.model.domain.model.{ObjectValue, PropertyReferenceValues}
...
class RealtimeObject(private[this] val value: ObjectValue,
  ...
  def dataValue(): ObjectValue = {
    model.ObjectValue(id, childValues.map { case (k, v) => k -> v.dataValue() })
  }
