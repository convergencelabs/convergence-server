...
private[ot] object StringSpliceSetTF extends OperationTransformationFunction[StringSpliceOperation, StringSetOperation] {
  def transform(s: StringSpliceOperation, c: StringSetOperation): (StringOperation, StringOperation) = {
    // S-RS-1
    (s.copy(noOp = true), c)
  }
}
