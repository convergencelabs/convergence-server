...
class PublishMessageEventProcessorSpec extends ScalaTestWithActorTestKit
  with AnyWordSpecLike
  with MockitoSugar
  with TestConstants {

  "An PublishMessageEventProcessor" when {
    ...
    "processing a request" must {
      "return success if the user is a member" in {
        ...
        response.task.reply.response shouldBe
          PublishChatMessageResponse(Right(PublishChatMessageAck(newState.lastEventNumber, newState.lastEventTime)))
      }
    }
    ...
    "validating a request" must {
      "return success if the user is a member" in {
        val message = createMessage(member1.userId)
        PublishMessageEventProcessor.validateMessage(message, state) shouldBe Right(())
      }

      "return a failure response if the user not a member" in {
        val message = createMessage(nonMember)
        PublishMessageEventProcessor.validateMessage(message, state) shouldBe
          Left(PublishChatMessageResponse(Left(ChatActor.ChatNotJoinedError())))
      }
    }
    ...
    "creating an event" must {
      "create a proper event" in {
        val message = createMessage(member1.userId)
        val event = PublishMessageEventProcessor.createEvent(message, state)

        event.id shouldBe state.id
        event.user shouldBe message.requester
        event.message shouldBe message.message
    ...
    "persisting an event" must {
      "succeed when the persistence operations succeed" in {
        ...
        val event = ChatMessageEvent(1L, state.id, member1.userId, timestamp, "test")

        val result = PublishMessageEventProcessor.persistEvent(chatStore, permissionsStore)(event)
        result shouldBe Success(())

        Mockito.verify(chatStore).addChatMessageEvent(event)
      }

      "fail when the persistence operations fail" in {
        val chatStore = mock[ChatStore]
        Mockito.when(chatStore.addChatMessageEvent(Matchers.any())).thenReturn(Failure(InducedTestingException()))
    ...
    "updating state" must {
      "compute the correct state" in {
        val timestamp = Instant.now()
        val event = ChatMessageEvent(1L, state.id, member1.userId, timestamp, "test")
        val newState = PublishMessageEventProcessor.updateState(event, state)
    ...
    "creating a success reply" must {
      "compute the correct reply" in {
        ...
        val timestamp = Instant.now()
        val event = ChatMessageEvent(1L, state.id, message.requester, timestamp, message.message)

        val task = PublishMessageEventProcessor.createSuccessReply(message, event, state)
        task.reply.replyTo shouldBe message.replyTo
        task.reply.response shouldBe PublishChatMessageResponse(Right(PublishChatMessageAck(event.eventNumber, event.timestamp)))
        task.broadcast shouldBe Some(ChatClientActor.RemoteChatMessage(
          event.id, event.eventNumber, event.timestamp, event.user, event.message))
    ...
    "creating an error reply" must {
      "compute the correct reply" in {
        val reply = PublishMessageEventProcessor.createErrorReply(UnknownError())
        reply shouldBe PublishChatMessageResponse(Left(UnknownError()))
      }
    }
  }
