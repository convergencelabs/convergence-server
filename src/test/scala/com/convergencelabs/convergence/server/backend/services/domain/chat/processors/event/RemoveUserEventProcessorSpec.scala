  ...
  "An RemoveUserEventProcessor" when {
    ...
    "processing a request" must {
      "return success if the user is not a member" in {
        ...
        Mockito.when(permissionsStore.removeAllPermissionsForUserFromTarget(Matchers.any(), Matchers.any()))
          .thenReturn(Success(()))
    ...
    "persisting an event" must {
      "succeed when the persistence operations succeed" in {
        ...
        Mockito.verify(permissionsStore).removeAllPermissionsForUserFromTarget(event.userRemoved, ChatPermissionTarget(state.id))
      }
