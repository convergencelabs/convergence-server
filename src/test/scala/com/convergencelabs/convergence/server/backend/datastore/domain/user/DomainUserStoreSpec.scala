...
class DomainUserStoreSpec
    extends PersistenceStoreSpec[DomainUserStore](NonRecordingSchemaManager.SchemaType.Domain)
  ...
  "A DomainUserStore" when {
    ...
    "validating credentials" must {
      "return true for a vaid usename and password" in withPersistenceStore { store =>
        initUsers(store)
        store.validateNormalUserCredentials(User1.username, "password").get shouldBe true
      }

      "return false for an valid username and invalid password" in withPersistenceStore { store =>
        initUsers(store)
        store.validateNormalUserCredentials(User1.username, "wrong").get shouldBe false
      }

      "return false for an invalid username" in withPersistenceStore { store =>
        initUsers(store)
        store.validateNormalUserCredentials("no one", "p").get shouldBe false
      }
    }
