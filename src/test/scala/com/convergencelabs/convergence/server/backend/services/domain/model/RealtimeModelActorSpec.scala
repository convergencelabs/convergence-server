...
class RealtimeModelActorSpec
  ...
  trait TestFixture {
    ...
    Mockito.when(persistenceProvider.collectionPermissionsStore.getUserPermissionsForCollection(collectionId)).thenReturn(Success(Map[DomainUserId, CollectionPermissions]()))
