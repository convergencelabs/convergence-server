...
class RealtimeModelActorSpec
  ...
  trait TestFixture {
    ...
    Mockito.when(persistenceProvider.collectionPermissionsStore.getCollectionUserPermissions(collectionId)).thenReturn(Success(Map[DomainUserId, CollectionPermissions]()))
