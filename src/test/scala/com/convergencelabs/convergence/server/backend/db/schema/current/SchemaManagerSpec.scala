...
import com.convergencelabs.convergence.server.backend.db.schema.current.SchemaManager.{DeltaApplicationError, DeltaValidationError, RepositoryError, StatePersistenceError}
import com.convergencelabs.convergence.server.backend.db.schema.current.SchemaMetaDataRepository.{FileNotFoundError, ParsingError, UnknownError}
import com.convergencelabs.convergence.server.backend.db.schema.current.delta.Delta
import org.mockito
...
final class SchemaManagerSpec extends AnyWordSpecLike with Matchers with MockitoSugar {
  ...
  "SchemaManager" when {
    "installing a fresh schema to the latest version" must {
      "apply and record the correct deltas " in {
        ...
        Mockito.when(schemaStatePersistence.recordImplicitDeltasFromInstall(List(InstallImplicitDelta)))
          .thenReturn(Success(()))
        ...
        verify(schemaStatePersistence, times(1))
          .recordImplicitDeltasFromInstall(List(InstallImplicitDelta))
      }
  ...
  "upgrading an existing schema" must {
    "apply and record the correct deltas" in {
      ...
      Mockito.when(schemaStatePersistence.recordDeltaSuccess(Delta1))
        .thenReturn(Success(()))

      Mockito.when(schemaStatePersistence.recordDeltaSuccess(Delta2))
        .thenReturn(Success(()))
      ...
      verify(schemaStatePersistence, times(1))
        .recordDeltaSuccess(Delta1)

      verify(schemaStatePersistence, times(1))
        .recordDeltaSuccess(Delta2)
    }
    ...
    "return a delta application failure and record the failure if applying a delta fails" in {
      ...
      verify(schemaStatePersistence, times(1))
        .recordDeltaFailure(mockito.Matchers.eq(Delta1), mockito.Matchers.any())
    }
...
object SchemaManagerSpec extends MockitoSugar {
  ...
  val InstallImplicitDelta: UpgradeDeltaEntry =
    UpgradeDeltaEntry("_implicit_delta", "", Some(false), None)
  ...
  val Delta1Entry: UpgradeDeltaEntry =
    UpgradeDeltaEntry("delta1", DeltaHash, Some(false), None)

  val Delta2Entry: UpgradeDeltaEntry =
    UpgradeDeltaEntry("delta2", DeltaHash, Some(false), None)
